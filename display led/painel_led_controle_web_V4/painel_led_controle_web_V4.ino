...
const char* ARQUIVO_BRILHO = "/brilho.txt"; // guarda o ultimo brilho definido

String nomeFiguraAtual = ""; // nome (sem extensao) da figura atualmente no painel
uint8_t brilhoAtual = BRIGHTNESS; // brilho atual do painel (1-255)
...
// ---------------------------------------------------------------------
// Persistência do brilho (LittleFS)
// ---------------------------------------------------------------------
void salvarBrilho(uint8_t valor) {
  File f = LittleFS.open(ARQUIVO_BRILHO, "w");
  if (f) {
    f.print(valor);
    f.close();
  }
...
void carregarBrilhoInicial() {
  if (LittleFS.exists(ARQUIVO_BRILHO)) {
    File f = LittleFS.open(ARQUIVO_BRILHO, "r");
    String texto = f.readString();
    f.close();
    texto.trim();
    int valor = texto.toInt();
    if (valor >= 1 && valor <= 255) {
      brilhoAtual = (uint8_t)valor;
      Serial.printf("Brilho inicial carregado: %d\n", brilhoAtual);
      return;
    }
  }
  brilhoAtual = BRIGHTNESS; // valor padrao definido no topo do arquivo
}
...
<style>
  ...
  #sliderBrilho { width: 220px; vertical-align: middle; }
  #valorBrilho { display:inline-block; min-width:32px; font-size:14px; }
</style>
</head>
<body>
...
<h3>Brilho do painel</h3>
<div class="barra">
  <input type="range" id="sliderBrilho" min="5" max="255" value="40">
  <span id="valorBrilho">40</span>
</div>
...
// --- Brilho do painel ---
const sliderBrilho = document.getElementById('sliderBrilho');
const valorBrilho = document.getElementById('valorBrilho');

sliderBrilho.addEventListener('input', () => {
  valorBrilho.innerText = sliderBrilho.value;
});

async function enviarBrilho() {
  try {
    const resp = await fetch('/brilho?valor=' + sliderBrilho.value, { method: 'POST' });
    estadoTexto.innerText = resp.ok ? ('Brilho ajustado para ' + sliderBrilho.value) : 'Erro ao ajustar brilho.';
  } catch (e) {
    estadoTexto.innerText = 'Erro de conexao.';
  }
}

// envia só quando o usuário solta o controle (mouseup/touchend), não a cada pixel arrastado
sliderBrilho.addEventListener('change', enviarBrilho);

// carrega o brilho atual salvo no ESP32 ao abrir a pagina
fetch('/brilho').then(r => r.text()).then(valor => {
  sliderBrilho.value = valor;
  valorBrilho.innerText = valor;
}).catch(() => { /* mantem o valor padrao do slider */ });
...
void handleBrilhoGet() {
  server.send(200, "text/plain", String(brilhoAtual));
}

void handleBrilhoSet() {
  String texto = server.arg("valor");
  int valor = texto.toInt();
  if (valor < 1) valor = 1;
  if (valor > 255) valor = 255;

  FastLED.setBrightness((uint8_t)valor);
  FastLED.show();
  salvarBrilho((uint8_t)valor);
...
void setup() {
  ...
  // --- Carrega o ultimo brilho salvo ---
  carregarBrilhoInicial();
  ...
  server.on("/brilho", HTTP_GET, handleBrilhoGet);
  server.on("/brilho", HTTP_POST, handleBrilhoSet);
  server.begin();
